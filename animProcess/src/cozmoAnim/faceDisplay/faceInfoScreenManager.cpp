...
namespace Anki
{
  namespace Vector
  {
    ...
    FaceInfoScreenManager::FaceInfoScreenManager()
        : _scratchDrawingImg(new Vision::ImageRGB565()), _wheelMovingForwardsCount(0), _wheelMovingBackwardsCount(0), _liftTriggerReady(false), _headTriggerReady(false), _debugInfoScreensUnlocked(false), _currScreen(nullptr), _webService(nullptr)
    {
...
#define SET_EXIT_ACTION(screen, lambda) \
  GetScreen(ScreenName::screen)->SetExitScreenAction(lambda);
      ...
      // * Camera
      const bool hideSpecialDebugScreens = false; // TODO: Use this line in master
      ...
      ADD_SCREEN_WITH_TEXT(paytondev, Main, {"PAYTON DEVELOPSER"});
      ...
      ADD_MENU_ITEM(paytondev, "ACKNOWLEDGE", None);
      // === Self test screen ===
    ...
    void FaceInfoScreenManager::DrawFAC()
    {
      DrawTextOnScreen({"FAC"},
                       NamedColors::BLACK,
                       (Factory::GetEMR()->fields.PLAYPEN_PASSED_FLAG ? NamedColors::GREEN : NamedColors::RED),
        ...
        bool triggerRecognized)
    {
      ...
      for (int i = 0; i < 12; ++i)
      {
        ...
        drawImg.DrawFilledConvexPolygon({{baseX + barWidthFactor[i][0].x(), baseY + barWidthFactor[i][0].y()},
      ...
      drawImg.DrawFilledCircle({(float)(center_px.x() + (int)(barLenFactor[winningIndex].x() * (float)(circleRadius_px + 1.f))),
      ...
      {
        drawImg.DrawFilledCircle({(float)FACE_DISPLAY_WIDTH - activeCircleRad_px,
      ...
      {
        drawImg.DrawFilledConvexPolygon({{FACE_DISPLAY_WIDTH - triggerDispWidth_px,
      ...
      drawImg.DrawFilledConvexPolygon({{buffFullBarWidth_px, FACE_DISPLAY_HEIGHT - endOfBarHeight_px},
      ...
      drawImg.DrawFilledConvexPolygon({{0, FACE_DISPLAY_HEIGHT - endOfBarHeight_px + buffFullBarHeight_px / 2},
...
#if ANKI_DEV_CHEATS
      if (kFakeButtonPressType == 1)
      { // single press
        singlePressDetected = true;
        kFakeButtonPressType = 0;
      }
      else if (kFakeButtonPressType == 2)
      { // double press
        doublePressDetected = true;
        kFakeButtonPressType = 0;
      }
    ...
    void FaceInfoScreenManager::DrawMain()
    {
      ...
      ColoredTextLines lines = {
          {{serialNo}, {hwVer, NamedColors::WHITE, false}},
          {osVer},
          {ssid},
#if FACTORY_TEST
          {"IP: " + ip},
    ...
    void FaceInfoScreenManager::DrawNetwork()
    {
      ...
      auto getStatusString = [](const auto &status)
      {
        switch (status)
        {
        case CloudMic::ConnectionCode::Available:
        {
          return ColoredText("AVAILABLE", NamedColors::GREEN);
        }
        case CloudMic::ConnectionCode::Connectivity:
        {
          return ColoredText("CONNECTIVITY", NamedColors::RED);
        }
        case CloudMic::ConnectionCode::Tls:
        {
          return ColoredText("TLS", NamedColors::RED);
        }
        case CloudMic::ConnectionCode::Auth:
        {
          return ColoredText("AUTH", NamedColors::RED);
        }
        case CloudMic::ConnectionCode::Bandwidth:
        {
          return ColoredText("BANDWIDTH", NamedColors::RED);
        }
        default:
        {
          return ColoredText("CHECKING...", NamedColors::BLUE);
        }
        }
      };
